  ...
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="删除标记" prop="isDelete" hidden >
          <el-input v-model="form.isDelete" style="width: 370px;" />
        </el-form-item>

        <el-form-item label="行记录版本" prop="version" hidden >
        ...
        <el-form-item v-if="form.pid !== 0" style="margin-bottom: 0;" label="上级分类" prop="pid">
          <treeselect v-model="form.pid" :options="classifys" style="width: 370px;" placeholder="选择上级分类" />
        </el-form-item>
    ...
    <el-table ref="table" v-loading="crud.loading" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" default-expand-all :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-permission="['admin','classify:edit','classify:del']" label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <udOperation
            :data="scope.row"
...
<script>
import crudClassify from '@/api/system/skuclassify'
...
export default {
  ...
  methods: {
    ...
    changeEnabled(data, val) {
      this.$confirm('此操作将 "' + this.dict.label.job_status[val] + '" ' + data.name + '商品分类, 是否继续？', '提示', {
